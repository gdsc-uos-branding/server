...
public class SecurityConfig {
    ...
    public SecurityConfig(AuthService authService, ApplicationProperties applicationProperties) {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.authorizeRequests()
                .antMatchers(applicationProperties.getWhiteListURLs().toArray(new String[0])).permitAll() // 모든 사용자가 접근 가능하게 허용, 로그아웃 후 새로고침 시 바로 소셜 로그인이 가능한 화면이 뜨는 것을 막기 위함
